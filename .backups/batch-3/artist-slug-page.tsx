...
export default async function ArtistDetailPage({ params }: Props) {
  ...
  const profile = await mediaRepository.getPublicArtistProfileMedia(artist);
  ...
  return (
    <EditorialDetail eyebrow="Artist" title={artist.name_en} subtitle={artist.name_ar} image={profile ? { src: profile.url, alt: profile.alt_en || artist.name_en } : null}
      facts={[{ label: 'Nationality', value: artist.nationality_en }, { label: 'Born', value: artist.birth_year > 1900 ? artist.birth_year : undefined }, { label: 'Representation', value: artist.representation_status?.replaceAll('_', ' ') }, { label: 'Instagram', value: artist.instagram }]}
